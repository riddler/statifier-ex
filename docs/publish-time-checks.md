...
| S5 | A host-registered send processor cannot deliver: `error.communication` carrying `sendid`, written by the host | `Statifier.Session.failed_send/3`; `Statifier.Interpreter.deliver_internal/5` when no session process is running | `docs/adr/0069-host-registered-send-types.md` (decision 5) | depends on the processor: RT1 to RT9, RT18, RT19 and RT22 are the router's | NONE in statifier, which cannot see a processor's routes; the router's twins are in those rows |
