...
- **ADR judge** (`mix adr.judge`): an LLM-backed stage scoped to ADR-0012,
  gated on the `claude` CLI being on `PATH` and on the diff touching
  `lib/statifier/` files (see Phase 2's "Reworked" note: the original design
  called the Anthropic API directly with an `ANTHROPIC_API_KEY`; the rework
  shells out to the developer's own `claude` CLI instead). A candidate
  finding only reaches gate-failure status if an independent refute pass
  fails to overturn it; otherwise it is dropped. Without the CLI, or with no
  core files in the diff, the stage reports itself skipped with a specific
  reason rather than failing or silently passing. It runs only under
  `--profile merge` (`/merge-request`'s gate step), not in the default full
  gate or `--profile loop`, since it is the one stage here that costs money
...
**Reworked (2026-08-05, same issue).** The first landing (commit `d6a300c`)
called the Anthropic Messages API directly through `Req` and an
`ANTHROPIC_API_KEY`. That needed a key nobody had provisioned for this repo
and put a real HTTP dependency in `mix.exs` for a stage most runs never use.
The rework shells out to the developer's own `claude` CLI instead
(`System.cmd/3`, `-p` for one non-interactive completion, `--output-format
json` for a parseable reply, `--tools ""` and `--strict-mcp-config` so the
call has no tool access and no MCP servers and so can only see the prompt
this module built), which rides the developer's existing Claude Code auth and
drops the `Req` dependency entirely. It also moves the stage out of the
default gate: `adr_judge: [enabled: false]` in `.quality.exs` disables it for
both a bare `mix quality` and `--profile loop`, and a new `:merge` profile
(`adr_judge: [enabled: true]`) re-enables it for the one path that wants
it - `/merge-request`, run before pushing. The sections below describe the
result, not the original design; where they differ from `d6a300c`, the
rework wins.

An open question the CLI shell-out surfaced empirically, not from reading
docs: `--tools ""` genuinely leaves the child session with no tools (verified
by inspecting its `system: init` event), but the model does not reliably
notice this on its own, and without an explicit instruction it would narrate
`<function_calls>` blocks it cannot execute rather than answering - burning
the whole completion on a hallucinated read/grep attempt. Both prompts now
open with "You have no tool access in this session" to head this off. It
mostly works; occasional rambling before the fenced verdict is still possible
and is why `extract_json/1` scans the whole response for the *last* fenced
block rather than assuming the reply is bare JSON or that a fence, if any,
wraps the whole thing - `"respond with JSON only"` is a request the CLI does
not always honor to the letter, and both the propose and refute passes were
observed, live, replying with a fenced answer preceded by prose. Assumed
rather than asked, since a human was not available: this is prompt-tuning to
compensate for text the model volunteers on its own, not something a
temperature/system-prompt flag controls; if it degrades in a future CLI
version, the parse failure is safe (both passes fail closed) even though the
signal loss would not be.
...
failing, when it has nothing to check or the `claude` CLI is unavailable.
...
**File**: `mix.exs`
**Changes**: None, after the rework. The original design added
`{:req, "~> 0.5", only: :dev, runtime: false}` for a direct Anthropic API
call; the CLI shell-out needs no HTTP client, so `Req` (and the
`Code.ensure_loaded?/1` guard it needed to keep `Req`-absent `:test` builds
compiling) never lands at all.
...
defmodule Mix.Statifier.AdrJudge do
  ...
  prompt to a response, real calls shelling out to the `claude` CLI in
  production, a stub in tests). `collect/1` gathers the diff the same way
...
- `collect/1` returns `:no_base_ref` (mirrors the other two guards), `:no_cli`
  when `System.find_executable("claude")` is `nil`, or `:no_core_changes`
  when the diff touches no `lib/statifier/` files - all three become a skip
  in the task, not a failure.
- The propose prompt includes the full text of ADR-0012 and the diff hunks
  ...
  each naming a `file`, `line`, and a one-sentence claim. Both prompts open
  with an explicit "you have no tool access" line - see the note above this
  section on why.
...
- Model name is `System.get_env("STATIFIER_ADR_JUDGE_MODEL", @default_model)`,
  passed to the CLI as `--model`, so a stronger model can be substituted
  without a code change.
- `call_claude_cli/1` runs `claude -p <prompt> --output-format json --tools ""
  --strict-mcp-config --model <model>` and hands the stdout to
  `parse_cli_response/1`, which reads the `"type": "result"` stream event
  (`"is_error": false` for success). `extract_json/1` then scans the model's
  own reply text for the *last* fenced code block (falling back to the
  trimmed reply when there is none), because the CLI routinely ignores
  "respond with JSON only" and answers with prose - sometimes several
  hallucinated tool-call fences - before its real, fenced verdict.
...
human-readable reason string (`"claude CLI not on PATH"`,
`"no lib/statifier/ files in this diff"`, the existing no-base-ref reason).

#### 4. Wire into the gate, disabled by default
...
Not added to the `:loop` profile - but that alone is not enough, because a
custom stage with no `profiles:` entry naming it still runs in a bare `mix
quality`, and this one makes real `claude` CLI calls that a full gate run
should not pay for unasked. Disable it at the top level instead,
`adr_judge: [enabled: false]`, and add a `:merge` profile that re-enables it:

```elixir
profiles: [
  loop: [stages: [:format, :compile, :credo, :test], test: [...]],
  merge: [adr_judge: [enabled: true]]
]
```

`--profile merge` then runs the full stage set (no `stages:` allow-list
narrows it) plus this one; a bare `mix quality` and `--profile loop` both
report it `skipped (disabled in .quality.exs)` / `skipped (not in profile
:loop)`. `/merge-request` runs `mix quality --profile merge` after the full
gate, before pushing.
...
The rework removed the `ANTHROPIC_API_KEY` requirement that made most of
these unreachable without a human provisioning a key, and a `claude` CLI
happens to be on `PATH` in the environment this rework was done in - so,
absent a human to hand these to, they were run for real rather than left
deferred:

- [x] With the `claude` CLI unavailable (`cli_available: false` injected),
      `mix adr.judge` reports skipped with reason `"claude CLI not on PATH"` -
      unit-tested directly (`test/mix/tasks/adr_judge_test.exs`).
- [x] With a diff touching only `docs/` and `.quality.exs` (this branch's own
      diff against `origin/main` at rework time), `mix quality --profile
      merge --quick` reported `○ ADR judge: skipped (no lib/statifier/ files
      in this diff)` for real, not stubbed.
- [x] Against the real `claude` CLI, with a throwaway `lib/statifier/`
      module whose moduledoc documented a removed `trace(:exit_set, state)`
      call: the propose pass reliably named it as a candidate. Running this
      is what surfaced the CLI-response-parsing bug the note above this
      Phase describes (a fenced or prose-then-fenced reply silently read as
      "no candidates"); after `extract_json/1` and the "no tool access"
      prompt line, both passes reliably return a real, not defaulted,
      JSON-decoded verdict end-to-end.
- [ ] **Not achieved live**: a hand-crafted violation surviving the refute
      pass to produce an actual `adr-0012-debuggability` finding against the
      real CLI. Every live attempt (haiku, the `STATIFIER_ADR_JUDGE_MODEL`
      default, and sonnet) constructed a good-faith refutation for a
      single-function throwaway module with no surrounding interpreter
      context to judge against - plausible model calibration (the design
      intentionally sets a high bar: ties go to "not a violation"), not an
      observed pipeline defect, since `test/mix/statifier/adr_judge_test.exs`
      already exercises the survives-refute path deterministically via a
      stub `opts[:caller]`. Left open rather than force-fit: reaching a real
      positive live would need either a bigger, more realistic diff or a
      stronger judge model, and no human was available to make that call.

**Implementation Note**: Use `mix quality --profile loop` between edits;
`mix quality` as the phase gate. The rework drops `Req`, so Dialyzer no
longer needs a new dep's PLT entries on the first post-rework run. Sabotage
each new pure test the same way as Phase 1.
...
2. Manually construct a small diff (e.g. a scratch file under `lib/statifier/interpreter.ex`
   ...
   scratch file.
3. With the `claude` CLI on `PATH` (no key needed after the rework), run
   `mix adr.judge --format json` against a hand-crafted diff that removes a
   trace-effect call from a stubbed interpreter function, to see the
   propose/refute pipeline run end-to-end against the real CLI. Done during
   the rework itself - see the Manual Verification checklist above and the
   note ahead of Phase 2's Overview for what it found.
...
This list originally required a human with a real `ANTHROPIC_API_KEY` and was
not achievable in an automated/offline setting. The 2026-08-05 rework
(Phase 2's "Reworked" note) replaced the direct Anthropic API call with a
`claude` CLI shell-out, which needs no key - only the CLI itself on `PATH`,
already true in this environment - so every item but the last was run for
real during the rework rather than staying deferred. See Phase 2's Manual
Verification checklist for what each one showed and the note ahead of Phase
2's Overview for the parsing bug the live run surfaced.

- [x] `claude` CLI unavailable is a clean skip, not a failure - unit-tested,
      no key or CLI involved either way.
- [x] A diff touching no `lib/statifier/` files is a clean skip - observed
      for real via `mix quality --profile merge --quick` on this branch's
      own (docs + `.quality.exs`-only) diff.
- [x] The propose/refute pipeline runs end-to-end against the real `claude`
      CLI without a key - observed for real with a throwaway
      `lib/statifier/` module; this is what found the markdown-fence /
      prose-preamble parsing bug `extract_json/1` now handles.
- [ ] **Still open**: a hand-crafted violation surviving refute to produce a
      real `adr-0012-debuggability` finding against the live CLI (not a stub).
      Every live attempt in this rework was overturned by the refute pass -
      plausible calibration for a context-free single-function throwaway
      module against a high, tie-goes-to-"not a violation" bar, not a
      pipeline defect, since the survives-refute path is deterministically
      exercised by `test/mix/statifier/adr_judge_test.exs`'s stub-`caller`
      tests. Needs either a more realistic diff or a stronger judge model to
      resolve live; no human was available to make that call, so it stays
      open rather than being forced.
