...
| [0056](0056-renumbered-adr-citations-pointers-move-history-stands.md) | After a renumbering, pointer citations move (path fix plus an at-the-time note) and historical statements stand; cross-repo ADR citations must name the repo | accepted (amended 2026-09-02: `<prefix>-ADR-NNNN` is the canonical compact form for decision 5's cross-repo citation, the prefix being the repository's beads prefix; existing forms stay valid, no sweep) |
...
| [0063](0063-caller-context-on-external-events-and-durable-timer-effects.md) | An opaque `caller_context :: term()` (default `nil`, never read by the library) rides `%Event{}` via `external/2`, is stamped by the core onto `%SendDelayed{}`/`%Cancel{}`, surfaces in four telemetry events' metadata, and recordings carry it (format 2 -> 3); ADR-0054's keys untouched | accepted (amends 0040 in part; amended 2026-09-01 by st-u9l6: `%Effect.Invoke{}`/`%Effect.CancelInvoke{}` join the carriers, their two telemetry events gain the key, recording format 3 -> 4) |
...
| [0068](0068-permanent-invoke-failure-is-a-suffixed-error-communication.md) | Permanent invoke failure is `error.communication.invoke.<invokeid>`, reported by the host through `Statifier.Session.failed_invocation/3` and delivered on the same invocation-tagged entry as `done.invoke` | accepted |
| [0069](0069-host-registered-send-types.md) | Host-registered send types fill the Event I/O Processor slot: a per-session `:send_types` map beside `:invoke_handlers`, an unregistered type refused in the core and by a pre-start check the host calls, a registered send handed to the host with a publicly built event, and ADR-0055's three owed items for the session route | accepted (amends 0047 decision 5 for `<send>`; amends 0054 decision 2 in part, scoped to built-in types; answers 0055 decision 3's trigger) |
| [0070](0070-statifier-emits-a-language-neutral-conformance-corpus.md) | statifier-ex emits a language-neutral conformance corpus under `conformance/`: the W3C suite redistributed under its 3-clause BSD arm with the notice, the corpus as the source the test generators read, a registry derived from `test/passing_tests.json`, claims per suite, a reserved `host` object and `statifier` suite, and a `--check` that fails on drift and on nothing to check | proposed (extends 0006) |
