defmodule Statifier.TestContent do
  ...
  2. It is the failing node the stop-on-error criterion needs: Phase 1's
     `<raise>`/`<log>` cannot fail, so a `fail: true` instance stands in for
     a node that returns `{:error, _}`, which is how the stop-on-error
     contract stays tested in this bead despite neither real node ever
     erroring.
  ...
  `%Machine{}` literal, so it stays inside st-wju.4's fixture rule.
  """
