defmodule Statifier.Document.Donedata do
  ...
  `Statifier.Document.Content.t()` when it has a `<content>` child. `<param>`
  as an alternative to `<content>` is not yet supported; only the
  static-content case is, so `content` is the only field besides `location`
  for now.
