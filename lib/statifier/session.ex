defmodule Statifier.Session do
  ...
  `message` is one of:
    ...
    - `{:unroutable, effect}` - a `:send`/`:send_delayed`/`:invoke` effect
      this session cannot route yet (see `Statifier.Session.Effects`'s own
      moduledoc for exactly which).
  ...
  and is forwarded, never guessed and never a crash. `#_internal`
  resolution, `#_parent`, `#_scxml_<sessionid>`, and external URI targets
  are a later bead's work; see `Statifier.Session.Effects`.
