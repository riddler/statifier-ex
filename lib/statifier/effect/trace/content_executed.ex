defmodule Statifier.Effect.Trace.ContentExecuted do
  ...
  `t:Statifier.Machine.Content.owner/0` (it describes where a content node
  ...
  `t:Statifier.Event.Cause.origin/0` embeds `Content.owner/0` unwidened for its
  ...
  - `{:global_script, index}` - a top-level `<script>` (spec 5.8), run at
    load time by `Statifier.Interpreter.run_global_script`. This case is
    ...
    block a content node lives *in*. `t:Statifier.Event.Cause.origin/0` already
    ...
    `t:Statifier.Compiler.Expressions.owner_ref/0` mints and
    `t:Statifier.Event.Cause.origin/0`'s own `{:global_script, index}` bullet
    already documents.
  ...
  load-time script) produced `c_indexes` - `t:Statifier.Machine.Content.owner/0`
